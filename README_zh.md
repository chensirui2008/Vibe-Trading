...
Vibe-Trading 为任何 MCP-compatible client 暴露 71 个 MCP tools。它作为 stdio subprocess 运行，无需 server setup。核心 research tools 对港股/美股/加密零 API key 可用；trading connector tools 使用当前选择的 connector profile；只有 `run_swarm` 需要 LLM key。
...
│   ├── mcp_server.py               # MCP server —— 71 个工具，面向 OpenClaw / Claude Desktop
│   │
│   ├── src/
