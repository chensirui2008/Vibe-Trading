...
<details>
<summary><b>Finance Skill Library</b> <sub>9 个类别中的 94 个 skills</sub></summary>

- 📊 94 个专业金融 skills，分布在 9 个类别中
- 🌐 覆盖传统市场、加密与 DeFi
- 🔬 从数据源到量化研究的完整能力链路
...
现有 stdio MCP Server，不额外启动 Web 服务，也不包含对话内自定义卡片。
...
全部 94 个 finance skills 都发布在 [open-space.cloud](https://open-space.cloud)，并通过 OpenSpace 的自进化引擎自主演进。
...
OpenSpace 会自动发现全部 94 个 skills，启用 auto-fix、auto-improve 和社区分享。在任意已连接 OpenSpace 的智能体中，可通过 `search_skills("finance backtest")` 搜索 Vibe-Trading skills。
...
│   │   │   ├── skills.py           #   skill loader（94 个内置 + 通过 CRUD 创建的用户 skill）
...
│   │   │
│   │   ├── skills/                 # 9 个类别共 94 个 finance skills（每个一份 SKILL.md）
