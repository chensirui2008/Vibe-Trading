...
- 股票池：优先使用用户指定股票池；否则使用能够逐项列明的美股股票池。必须报告股票池名称、成分日期、总数、成功取数数和失败代码。

禁止把 `screen_market` 返回的当日涨幅榜、热门股列表或当前指数成分代理冒充“美股全市场”。无法获得可审计股票池时，停止并请用户指定股票池；不要静默缩小范围。

## 工作流

### 1. 锁定数据与覆盖

1. 记录 `as_of`、股票池来源和成分日期。
2. 对每个代码取得至少 150 个完整交易日的日线 OHLCV。优先调用 Vibe-Trading 的 `get_market_data`，使用 `source="auto"`、`interval="1D"`，并确保 `max_rows` 不会对所需窗口做间隔抽样。
3. 检查重复日期、非递增日期、缺失 OHLCV、非正价格和覆盖不足。任何失败都进入 `failed_symbols`；不得用零值、前值填充或其他代码替代。
4. 若批量调用存在返回条数上限，先验证该上限是“每个代码”还是“整次请求”。分批时保持同一截止日和同一口径。
5. 若不同来源的价格历史不一致，报告来源与差异；不得拼接成一条伪连续序列。
